    ...
    public static int romanToInt(String s) {
        ...
        if(s.length() == 1) return map.get(s.charAt(0));
            ...
            for(int i = 0; i < s.length()-1; i++){
                String lastTwo = s.substring(i, i + 2);
                boolean calculate = calculate(lastTwo);
                if(calculate){
                    char first = lastTwo.charAt(0);
                    char second = lastTwo.charAt(1);
                ...
                if(i + 1 == s.length()-1 && !calculate(s.substring(i,i+2)))
